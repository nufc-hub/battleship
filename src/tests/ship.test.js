...
test('Orientation will be horizontal', () => {
  const ship = new Ship(3, true); // isHorizontal is set to true as a parameter

  expect(ship.isHorizontal).toBe(true);
});

test('Orientation will not be horizontal', () => {
  const ship = new Ship(3, false); // isHorizontal is set to false as a parameter
